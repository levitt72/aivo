...
class AudioBufferProcessor(FrameProcessor):
    ...
    audio can be obtained by registering an "on_audio_data" event handler. The
    event handler will be called every time `buffer_size` is reached or when the
    processor is cleaned up if `buffer_size` is 0.
    ...
    async def _call_on_audio_data_handler(self):
        ...
        merged_audio = self._merge_audio_buffers()
    ...
    def _merge_audio_buffers(self) -> bytes:
